...
# csv_path  = "./data/datasets/167_000008.csv"  # disabled by app       # ← 換成要測的軌跡
...
try:
    # 以索引排序，維持軌跡順序
    keep_idx_np = np.sort(keep_idx.numpy())
    simp_lat = df["lat"].iloc[keep_idx_np].to_numpy()
    simp_lon = df["lon"].iloc[keep_idx_np].to_numpy()

    # 1) Simplified-only polyline
    plt.figure(figsize=(6,6))
    if simp_lon.size > 1:
        plt.plot(simp_lon, simp_lat, '-', lw=2, color='tab:blue', label='simplified')
    else:
        plt.scatter(simp_lon, simp_lat, s=24, color='tab:blue', label='simplified')
    plt.axis("equal"); plt.title(Path(args.csv).stem + " — simplified only"); plt.legend()
    simp_line_path = Path("result/test_simplified.png")
    plt.savefig(simp_line_path, dpi=150, bbox_inches="tight")
    print(f" --> simplified line saved to {simp_line_path}")
    try: plt.show()
    except: pass

    # 2) Original vs simplified (both as polylines)
    plt.figure(figsize=(6,6))
    plt.plot(df["lon"], df["lat"], '-', lw=1.2, alpha=0.85, color='tab:blue', label='original')
    if simp_lon.size > 1:
        plt.plot(simp_lon, simp_lat, '-', lw=2.2, color='tab:orange', label='simplified')
    else:
        plt.scatter(simp_lon, simp_lat, s=24, color='tab:orange', label='simplified')
    plt.axis("equal"); plt.title(Path(args.csv).stem + " — compare (orig vs simplified)"); plt.legend()
    compare_path = Path("result/test_compare.png")
    plt.savefig(compare_path, dpi=150, bbox_inches="tight")
    print(f" --> compare plot saved to {compare_path}")
    try: plt.show()
    except: pass
except Exception as e:
