...
def graph_show(TIME_STAMP=False, annot_interval=1, use_map=True):
    ...
    WALK_DIR = os.path.join(os.getcwd(), 'walk_traj')
    df = pd.read_csv(os.path.join(WALK_DIR, '020_000001.csv'))
...
def data_statistics():
    ...
    if not os.path.isdir(WALK_DIR):
        raise RuntimeError(f"找不到資料夾：{WALK_DIR}")

    # 2. 掃所有 CSV
    csv_files = sorted([f for f in os.listdir(WALK_DIR) if f.lower().endswith(".csv")])
    num_files = len(csv_files)

    # 3. 統計總筆數
    total_points = 0
    for fn in csv_files:
        df = pd.read_csv(os.path.join(WALK_DIR, fn))
        total_points += len(df)
